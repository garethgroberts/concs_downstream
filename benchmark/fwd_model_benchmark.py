...
accumulator = D8Accumulator("d8_benchmark.asc")
...
source_concs = imageio.v2.imread('./concs_benchmark.tif').astype('double')
...
sources_on_profile = source_concs.flatten()[profile]
profile_coords = [accumulator.node_to_coord(node) for node in profile]

print("Visualizing results")
plt.figure(figsize=(12, 10))
plt.subplot(2, 2, 1)
plt.imshow(drainage_area, extent=accumulator.extent, cmap="Greys")
cb = plt.colorbar()
...
plt.subplot(2, 2, 2)
plt.imshow(source_concs, extent=accumulator.extent, cmap="viridis")
cb = plt.colorbar()
cb.set_label("Source concentration, mg/kg")
plt.xlabel("Easting (m)")
plt.ylabel("Northing (m)")
for channel in channels.coordinates:
    x, y = zip(*channel)
    plt.plot(np.double(x)+0.5, np.double(y)-0.5, c="blue")
    plt.plot(np.double(x)+0.5, np.double(y)-0.5, "bx")
...
plt.imshow(concs, extent=accumulator.extent, cmap="viridis")
cb = plt.colorbar()
cb.set_label("Concentration downstream, mg/kg")
plt.xlabel("Easting (m)")
plt.ylabel("Northing (m)")
plt.plot(
    np.double([x for x, _ in profile_coords])+0.5,
    np.double([y for _, y in profile_coords])-0.5,
    "kx",
)
plt.plot(
    np.double([x for x, _ in profile_coords])+0.5,
...
plt.plot(np.double([startx])+0.5, np.double([starty])-0.5, "ko", label="Start point")
plt.legend()
plt.title("Channel profile (planform)")

plt.subplot(2, 2, 4)
#plt.plot(distance, area_on_profile, "k-")
plt.plot(distance, concs_on_profile, "k-", label="Downstream")
plt.plot(distance, concs_on_profile, "kx")
plt.plot(distance, sources_on_profile, "bx")
plt.plot(distance, sources_on_profile, "b-", label="Source")
...
plt.ylim(min(concs_on_profile), max(sources_on_profile))
