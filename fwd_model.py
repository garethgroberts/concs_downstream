...
plt.subplot(2, 2, 2)
plt.imshow(source_concs, norm=LogNorm(), extent=accumulator.extent, cmap="viridis")
cb = plt.colorbar()
...
for channel in channels.coordinates:
    x, y = zip(*channel)
    plt.plot(np.double(x), np.double(y), c="blue")
...
plt.ylabel("Northing (m)")
plt.plot(
    np.double([x for x, _ in profile_coords]),
    np.double([y for _, y in profile_coords]),
    "k-",
    label="Channel profile",
)
plt.plot(np.double([startx]), np.double([starty]), "ko", label="Start point")
plt.legend()
